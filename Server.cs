                    ...
                    else if (waitingClient != remoteEndPoint)
                    {
                        byte[] messageFor1 =
                            Encoding.UTF8.GetBytes(
                                $"A pair of frens has been found! Please connect to {remoteEndPoint.Address}:{remoteEndPoint.Port}.");
                        byte[] messageFor2 =
                            Encoding.UTF8.GetBytes(
                                $"A pair of frens has been found! Please connect to {waitingClient.Address}:{waitingClient.Port}.");
